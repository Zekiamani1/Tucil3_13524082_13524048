...
func main() {
	...
	firstgrid, start, end, constraints, _ := core.CreateGrid(X, Y, matrix, costMatrix)
	if firstgrid != nil {
		firstgrid.PrintGrid()
	}
	...
	if choice == 1 {
		result = player.UCS(end, constraints)
	} else if choice == 2 {
		result = player.GBFS(end, constraints)
	} else if choice == 3 {
		result = player.ASTAR(end, constraints)
	} else {
		return
	}
